...
import rdtprotocolo.RedeSimulador;
...
public class Pacote
{
    ...
    public Pacote(int seq, int ack, int check, String newPayload)
    {
        seqnum = seq;
        acknum = ack;
        checksum = check;
        ...
        else if (newPayload.length() > RedeSimulador.MAXDATASIZE)
        {
            payload = null;
        }
        else
    ...
    public Pacote(int seq, int ack, int check)
    {
        seqnum = seq;
        acknum = ack;
        checksum = check;
